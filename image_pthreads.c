...
void* convoluteThread(void * argument_ptr) {

    struct arguments * args = (struct arguments *) argument_ptr;

    int threadNo = args->rank;
    Image* srcImage = args->srcImage;
    Image* destImage = args->destImage;

    int row,pix,bit,span;
    span=srcImage->bpp*srcImage->bpp;
    for (row=threadNo; row<srcImage->height; row += thread_count){
